// Tests for src/lib/queries/historical.ts — queryHistoricalStatsMultiMetric
// (H-10: the legacy single-metric queryHistoricalStats wrapper was removed —
// zero production callers; these tests now cover the live multi-metric query.)
...
describe('queryHistoricalStatsMultiMetric', () => {
  beforeEach(() => {
    mockQueryRaw.mockReset();
  });

  it('returns empty Map when historicalPeriods is empty', async () => {
    const r = await queryHistoricalStatsMultiMetric([], {});
  ...
  it('returns Map with mean, stdDev, n per outlet+item', async () => {
    ...
    expect(stats!.devBom.mean).toBe(0.05);
    expect(stats!.devBom.n).toBe(4);
    expect(stats!.devBom.stdDev).toBe(0); // variance = 0 when sumSq = n * mean^2
  });
  ...
  it('computes sample variance (N-1 Bessel correction) correctly', async () => {
    ...
    const stats = r.get('1|10')!;
    expect(stats.devBom.stdDev).toBeCloseTo(Math.sqrt(0.02), 5);
  });
  ...
  it('handles n=1 (variance = 0, no division by zero)', async () => {
    ...
    const stats = r.get('1|10')!;
    expect(stats.devBom.n).toBe(1);
    expect(stats.devBom.stdDev).toBe(0); // n=1 → variance = 0 (no division by zero)
  });

  it('handles multiple outlet+item pairs', async () => {
    mockQueryRaw.mockResolvedValueOnce([
      { outletId: 1, itemId: 10, devBomMean: 0.05, devBomSumSq: 0.01, devBomN: 3, wasteMean: 0, wasteSumSq: 0, wasteN: 0, susutMean: 0, susutSumSq: 0, susutN: 0, trialMean: 0, trialSumSq: 0, trialN: 0 },
      { outletId: 1, itemId: 20, devBomMean: 0.10, devBomSumSq: 0.03, devBomN: 3, wasteMean: 0, wasteSumSq: 0, wasteN: 0, susutMean: 0, susutSumSq: 0, susutN: 0, trialMean: 0, trialSumSq: 0, trialN: 0 },
      { outletId: 2, itemId: 10, devBomMean: 0.15, devBomSumSq: 0.07, devBomN: 3, wasteMean: 0, wasteSumSq: 0, wasteN: 0, susutMean: 0, susutSumSq: 0, susutN: 0, trialMean: 0, trialSumSq: 0, trialN: 0 },
    ]);
    const r = await queryHistoricalStatsMultiMetric(
      [{ monthLabel: 'Juli 2026', weekLabel: 'WEEK 1' }],
      {},
