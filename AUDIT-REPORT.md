...
### BUG-HUNT — sebar 4 agent pemburu bug atas UI terbaru (13e67a0) — `BH-1`
Permintaan user: *"Cari bug dengan sebar agent di UI terbaru ini"*. 4 agent paralel: **BUG-1** (audit diff VH-6+VH-7 baris-per-baris) · **BUG-2** (interior 5 tab) · **BUG-3** (layer eksekutif L0-L5 + cross-cutting) · **BUG-4** (runtime browser, harness mock-fetch 17 endpoint + matriks 7 tab × 375/768/1440 + dark mode). Hasil: **35 temuan terverifikasi (0 TINGGI runtime-crash, 2 TINGGI layout-mobile)** — semua bukti berbasis kutipan kode/pengukuran DOM.

**2 temuan TINGGI (runtime, hanya terlihat dengan data realistis):**
- **Area tab 375px: page scrollWidth 745px** (grid item tanpa `min-w-0` di AreaTab → min-content tabel 731px meledakkan track 351px). FIX: `min-w-0` pada 2 grid item + **ScrollBar horizontal kini dirender default di `ui/scroll-area.tsx`** (Radix set `overflow-x: hidden` saat tak ada scrollbar horizontal → kolom terpotong tak terjangkau; terbukti pasca-fix: `overflow-x: scroll` + scrollLeft berfungsi).
- **Pareto tab 375px: page scrollWidth 454px** (slot `action` SectionHeader — 2 Select `w-[120px]` — meluber; regresi VH-7 yang membuang `flex-wrap`). FIX: `flex-wrap` dipulihkan + `min-w-0` pada h2.

**SEDANG yang di-fix** (ringkas): ikon DeltaPill Minus kontra "+5,0%" (regresi VH-3) → ikon berbasis tanda · `formatQtySigned` menelan tanda minus Δ kuantitas · token `--chart-susut/trial/residual` MATI (violet/lime ter-hardcode di DeviationBreakdownChart — FIX #23 tak pernah diterapkan) · bar deviasi GrowthComparison positif=emerald padahal tooltip sendiri bilang "naik=memburuk" (IBCS) · error-state trend-chart mati tertutup "Menunggu peer data..." · 4 tabel klik-baris/sort-header tanpa akses keyboard (spread `clickableRowProps`/`sortableHeaderProps` baru di `lib/a11y.ts` — tanpa `role="button"` di `<th>` agar `aria-sort` tak mati) · `areaMultiplier.toFixed(1)` sisa dot-desimal · sisa EN "by deviation" ×3 · kolom QTY Waste/BOM diwarnai LOSS/SURPLUS (hijau untuk waste besar) · judul seksi "Trend Item" duplikat dengan CardTitle (regresi VH-7; sesuai §4 L6 modul ber-judul kuat cukup judulnya sendiri) · label bar signal-chart membuang tanda minus · `residualQty` mentah.

**RENDAH yang di-fix**: tabrakan ikon RotateCcw (Sinkron File vs Reset → FolderSync) · tombol Muat Ulang tanpa busy-state · `barColor` prop mati QuadrantCard (dihapus) · footer `toLocaleString()` tanpa `id-ID` · fallback `--dashboard-header-h` 104px → 141px · Ctrl+E/R tanpa guard `isTyping` · shortcut digit lolos saat Popover terbuka · `scroll-mt-32` < header 141px → `scroll-mt-40` · footer tanpa safe-area-inset · sweep EN/ID ±25 string (cached→cache, HIGH risk→risiko TINGGI, Historical Mean→Mean Historis, Metric/Peer Avg→Metrik/Rata-rata Peer, SELECTED→TERPILIH, "N out"→"N resto", dst.) · tombol Coba Lagi di 4 error-state (trend, flip ranking, item peer comparison, price effect) · key indeks → key stabil ×3 · `aria-expanded`/`aria-pressed` di PrioritySummaryCard + HistoricalZScoreCard · ~20 hex chart → token `var(--chart-*)`/`var(--muted-foreground)` (signal-chart, ItemTrendLineChart, ItemTrendRankChart, AnalysisCards, trend-chart) · Kontribusi heatmap 84.2% → 84,2% · badge "Memuat Rank" biru → amber.

**Verifikasi BH-1**: 4 gerbang (`tsc` 0 · vitest 446/446 · eslint **0 err/298 warn — di bawah baseline 299** karena prop mati barColor hilang · `next build` sukses) + **browser 22/22 asersi PASS** (verifyfix.js, harness mock yang sama): 7 tab × {375,1440} scrollWidth = viewport (Area 745→375, Pareto 454→375) · tabel Area terjangkau horizontal · 5 Select Pareto dalam viewport · footer sticky · selektor dimensi berfungsi · keyboard 3/6 + `?tab=` · judul Trend Item tidak lagi dobel · **nol console/hydration/page error nyata** · review VLM 3 screenshot (Area-375, Pareto-375, Area-1440): tanpa defect.
...
### UXFIX-1 — 4 permintaan UX user (lihat semua · drill-down · tooltip ganda · price effect) — `UXFIX-1`
Permintaan user 2025-12 (verbatim diringkas): (1) tombol "lihat semua" di Resto/Item Prioritas tidak bereaksi — hapus + tambahkan filter resto di Tab Resto; (2) hapus drill-down "Breakdown 15 Sinyal Priority Score"; (3) tooltip jadikan satu saja, isi = "ini buat apa" bahasa mudah; (4) tambahkan Price Effect ke Growth Comparison + drill-down item yang harganya naik (+ konfirmasi formula harga). **NOL perubahan perhitungan/query/API backend** — murni presentation + 1 hook fetch FE yang memakai endpoint `/api/price-effect` yang sudah ada.

**Yang diimplementasi:**
1. **Tombol "lihat semua" dihapus** (UX-NAVLINK-1): footer ItemPriorityPanel + RestoRecommendationCard kini caption coverage saja ("Top N item — dampak finansial" / "Top N dari X outlet — Y% dari deviasi"); `setActiveTab` tidak lagi dipakai kedua kartu. Tab Item/Resto tetap reachable dari tab bar.
2. **Filter Resto di Tab Resto** (UX-RESTOFILTER-1): `SearchableComboBox` di dua tempat — bar atas tab (Card kompak, ikon Store, sm:w-80) + di dalam empty-state "Pilih outlet" (max-w-sm). Sumber daftar = `useStatus` cache (nol request baru) dengan filter konsistensi area/pic/kelompok SAMA dengan FilterBar (BUG-FE-2 — dropdown tak pernah menawarkan outlet yang disembunyikan filter aktif). Pemilihan → `setFocusOutlet` (scope TAB SAJA — tidak merefilter seluruh dashboard seperti outlet FilterBar global; paritas semantik dengan klik baris Resto Prioritas). Opsi "Semua / Reset pilihan" mengembalikan empty-state.
3. **Drill-down Breakdown 15 Sinyal dihapus** (UX-DRILLDOWN-1): akordeon per-sinyal + `SignalChart` ilustratif + disclaimer "chart di bawah adalah ilustrasi" hilang — breakdown kini list statis (dot, ikon, skor, kontribusi, nilai, badge; penjelasan sinyal bertahan sebagai `title` hover per baris). Tetap utuh: toggle breakdown, Kontributor Teratas, header grup, baris Total (FIX CHART-2). Modul mati `priority-summary/signal-chart.tsx` + `chart-data-builders.ts` DIHAPUS (−17 warning eslint); prop `outletItems` di-drop dari call site RestoAnalysis.
4. **Tooltip tunggal bahasa mudah** (UX-TOOLTIP-1): `FormulaInfo` di-restyle jadi SATU tooltip "ini buat apa" — paragraf UNTUK APA/CARA BACA/CONTOH/DRILL-DOWN (label bold, body /85), rumus hanya footnote mono kecil "Rumus: …" (−heading "Rumus Perhitungan" + kotak formula). 3 kartu yang tadinya menampilkan DUA ikon ⓘ berdampingan digabung: GrowthComparison, TopGrowthCard, PriceEffectCard (konten kedua tooltip digabung & disederhanakan). Catatan teknis: `\n` multi-baris wajib bentuk ekspresi `{'…'}` (JSXText tidak memproses escape) — konvensi lama AnalysisCards dipertahankan.
5. **Growth Comparison + Harga (AVG)** (UX-PRICE-1): hook baru `src/hooks/usePriceEffect.ts` (pola useRecommendations — queryKey builder tunggal + `?? null` normalisasi; staleTime 5 mnt/gcTime 10/keepPreviousData) mengambil alih inline fetch PriceEffectCard sehingga KEDUA konsumen share SATU request per scope (TanStack dedupe). GrowthComparison menambah metric ke-5 "Harga (AVG)" = `avgPriceChangePct/100` (backend mengirim satuan persen; chart memakai rasio — dikonversi); bar naik = amber (tekanan harga), turun = emerald (paritas semantik deviasi BUG-3-04/IBCS). Badge top-driver metric harga = item dengan |priceEffect| terbesar di antara yang naik (+ fallback turun). Drill-down metric harga = panel "item yang harganya naik" (urut |priceEffect|, bar share, +X% harga amber, efek harga Rp, klik → ItemDeepDive via `clickableRowProps`) + seksi "harga turun" (top 5) + footnote formula & caveat "kenaikan harga ≠ pemborosan"; honesty note saat `itemsPriceUp` > item yang ditampilkan (API top-20 by |netDelta|).

**Konfirmasi formula harga (pertanyaan user)**: YA — harga implisit per item = **Σ|nominalDeviasi| / Σ|qtyDeviasi|** per periode (nasional, dirata-ratakan; `price-effect.ts:190-191` — nCurr/qCurr dengan n=Σ|nominalDeviasi|, q=Σ|qtyDeviasi|). Efek harga Bennet = (Pc−Pp) × rata-rata(Qc,Qp) — eksak tanpa residual.

**Verifikasi UXFIX-1**: 4 gerbang (`tsc` 0 · vitest 446/446 · eslint 0 err/333 warn — turun dari baseline 350 karena modul mati dihapus, 0 warning baru via stash-compare · `next build` sukses). Browser end-to-end tidak dapat dijalankan pada clone audit ini (tanpa `.env`/DB) — verifikasi statis + 4 gerbang.
