...
- **Fitur analisa "Kepatuhan" (PAKET E — FEAT-COMPLIANCE-1..9)** — lihat seksi "PAKET E" di atas. Ringkas: tab ke-6 dengan 6 lensa kontrol + 3 lensa turunan/pendamping — kepatuhan toleransi per item (paritas penuh rule engine via RuntimeThresholds bersama), prioritas penetapan toleransi (item tanpa toleransi berdeviasi besar), deviasi tak terjelaskan (residual) per outlet dengan klasifikasi WARN/HIGH, |deviasi|/penjualan per outlet, split BAHAN vs PACKAGING, indikasi transfer antar outlet (item×area loss↔surplus serentak + outlet terbesar per sisi + "nilai cocok"), **ketidakcocokan antar-area** (item loss di area A ↔ surplus di area B — pairing di lapisan shaping dari baris `transfer_agg` yang sama, nol scan/round-trip tambahan), **kronis vs sekali-timu per outlet** (lensa level bulan via route `/api/chronic-outlets` terpisah — KRONIS = deviasi ≥75% minggu, SPIKE = minggu terburuk ≥60% |dev| bulanan; FE query kedua keyed month-only sehingga ganti minggu tidak refetch), dan **kualitas input angka bulat** (share |qtyDeviasi| berakhir 0/5 per outlet vs baseline periode — indikator menaksir vs menghitung; 3 kolom FILTER di scan yang sama). Route `/api/compliance` + `/api/chronic-outlets` (cache 5 mnt, rate-limit 30/mnt, maxDuration 60); FE keep-alive + staleTime 5 mnt + invalidasi refresh `['compliance']` + `['chronic-outlets']`. Verifikasi: `tsc --noEmit` 0 error, eslint 0 error (0 warning baru), vitest 438/438.
