...
**Route** `/api/compliance`: GET `month`+`week` wajib + filter `area/kelompok/outlet/pic` (regex + resolusi kelompok∩PIC paritas flip-ranking), rate-limit 30/mnt, cache DB 5 mnt (`withCacheAndDedup`, SWR stale flag), `maxDuration` 60. **FE**: tab ke-6 "Kepatuhan" (lazy import + `forceMount` keep-alive; strip 6 KPI ringkasan + 7 section tabel; TanStack `staleTime` 5 mnt + `gcTime` 10 mnt + `keepPreviousData`; tombol refresh meng-invalidate key `['compliance']`).
...
- **Fitur analisa "Kepatuhan" (PAKET E — FEAT-COMPLIANCE-1..7)** — lihat seksi "PAKET E" di atas. Ringkas: tab ke-6 dengan 6 lensa kontrol + 1 lensa turunan dari SATU scan periode — kepatuhan toleransi per item (paritas penuh rule engine via RuntimeThresholds bersama), prioritas penetapan toleransi (item tanpa toleransi berdeviasi besar), deviasi tak terjelaskan (residual) per outlet dengan klasifikasi WARN/HIGH, |deviasi|/penjualan per outlet, split BAHAN vs PACKAGING, indikasi transfer antar outlet (item×area loss↔surplus serentak + outlet terbesar per sisi + "nilai cocok"), dan **ketidakcocokan antar-area** (item loss di area A ↔ surplus di area B — pairing di lapisan shaping dari baris `transfer_agg` yang sama, nol scan/round-trip tambahan). Route `/api/compliance` (cache 5 mnt, rate-limit 30/mnt, maxDuration 60); FE keep-alive + staleTime 5 mnt + invalidasi refresh `['compliance']`. Verifikasi: `tsc --noEmit` 0 error, eslint 0 error (0 warning baru), vitest 438/438.
