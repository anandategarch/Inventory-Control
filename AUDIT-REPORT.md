...
**Route** `/api/compliance`: GET `month`+`week` wajib + filter `area/kelompok/outlet/pic` (regex + resolusi kelompok∩PIC paritas flip-ranking), rate-limit 30/mnt, cache DB 5 mnt (`withCacheAndDedup`, SWR stale flag), `maxDuration` 60. **Route** `/api/chronic-outlets`: GET `month` wajib (month-grain, tanpa `week` — strict zod menolak param liar), pola rate-limit/cache/maxDuration identik. **FE**: tab ke-6 "Kepatuhan" (lazy import + `forceMount` keep-alive; strip 6 KPI ringkasan + 8 section tabel; TanStack `staleTime` 5 mnt + `gcTime` 10 mnt + `keepPreviousData`; tombol refresh meng-invalidate key `['compliance']` + `['chronic-outlets']`).
...
- **Fitur analisa "Kepatuhan" (PAKET E — FEAT-COMPLIANCE-1..8)** — lihat seksi "PAKET E" di atas. Ringkas: tab ke-6 dengan 6 lensa kontrol + 2 lensa turunan/pendamping — kepatuhan toleransi per item (paritas penuh rule engine via RuntimeThresholds bersama), prioritas penetapan toleransi (item tanpa toleransi berdeviasi besar), deviasi tak terjelaskan (residual) per outlet dengan klasifikasi WARN/HIGH, |deviasi|/penjualan per outlet, split BAHAN vs PACKAGING, indikasi transfer antar outlet (item×area loss↔surplus serentak + outlet terbesar per sisi + "nilai cocok"), **ketidakcocokan antar-area** (item loss di area A ↔ surplus di area B — pairing di lapisan shaping dari baris `transfer_agg` yang sama, nol scan/round-trip tambahan), dan **kronis vs sekali-timu per outlet** (lensa level bulan via route `/api/chronic-outlets` terpisah — KRONIS = deviasi ≥75% minggu, SPIKE = minggu terburuk ≥60% |dev| bulanan; FE query kedua keyed month-only sehingga ganti minggu tidak refetch). Route `/api/compliance` (cache 5 mnt, rate-limit 30/mnt, maxDuration 60); FE keep-alive + staleTime 5 mnt + invalidasi refresh `['compliance']` + `['chronic-outlets']`. Verifikasi: `tsc --noEmit` 0 error, eslint 0 error (0 warning baru), vitest 438/438.
