...
---
Task ID: REMOVE-AI
Agent: AI Features Remover
Task: Remove AI Executive Summary + AI Pattern Insight + LLM Narrative

Work Log:
- Read worklog.md (last 50 lines) for context — confirmed prior AUDIT-GROWTH-SIGNED entries about narrative.ts reading s.nominalDeviasi.growth (signed) at lines 90/136/335; with AI narrative removed entirely, this inconsistency auto-resolves (no more LLM consumer of those values).
- Read all 7 target files: src/engine/narrative/narrative.ts (438 lines), src/app/api/export-report/route.ts (982 lines), src/app/api/analysis/route.ts (826 lines), src/components/dashboard/ExportDialog.tsx (143 lines), src/app/page.tsx (651 lines), src/components/dashboard/Narrative.tsx (83 lines), src/hooks/useAnalysis.ts (237 lines).
- src/engine/narrative/narrative.ts — REWROTE entire file (438→74 lines). Deleted: import ZAI from 'z-ai-web-dev-sdk'; LLM_TIMEOUT_MS + withTimeout helper; NarrativeInput interface; SYSTEM_PROMPT; fmtPct + fmtNum helpers (only used by deleted LLM functions); buildStructuredSummary; buildFallbackNarrative; CFG_FACTOR; generateNarrative; EXEC_SUMMARY_PROMPT; generateAIExecutiveSummary; buildFallbackExecSummary; PATTERN_PROMPT; generateAIPatternInsight; buildFallbackPatternInsight. KEPT: buildRecommendations (rule-based, only uses InvestigationItem type). Imports simplified to `import type { InvestigationItem } from '@/types/inventory';`.
- src/app/api/export-report/route.ts — Edit 1: import line trimmed from `generateNarrative, buildRecommendations, generateAIExecutiveSummary, generateAIPatternInsight` to `buildRecommendations`. Edit 2: removed `topAnomaliesForNarrative: any[] = []` declaration (line 393). Edit 3: removed `if (topAnomaliesForNarrative.length < 5 && top.severity !== 'NORMAL') { topAnomaliesForNarrative.push(...) }` block (was lines 415-417). Edit 4: removed `narrativeInput` + `narrativePromise` + `aiSummaryPromise` + `aiInsightPromise` construction (was lines 544-550). Edit 5: removed `const [narrativeResult, aiSummary, aiInsight] = await Promise.all(...)` + `const { narrative, source: narrativeSource } = narrativeResult;` (was lines 560-561). Edit 6: removed `narrative, narrativeSource, aiSummary, aiInsight` from data object (was lines 584-587). Edit 7: removed Section "RINGKASAN EKSEKUTIF" render block — `if (hasSection('aiSummary') && data.aiSummary) { ... }` (was lines 634-642). Edit 8: removed Section "OBSERVASI & POLA DEVIASI" render block — `if (hasSection('aiInsight') && data.aiInsight) { ... }` (was lines 867-875). Edit 9: removed Section 15 "NARASI ANALISIS" render block — `if (hasSection('narrative')) { if (data.narrative ...) { ... } }` (was lines 877-890). Edit 10 (cleanup): removed now-unused `renderMarkdownLine` helper function (was lines 145-163) — its only callers were the deleted aiSummary/aiInsight sections. Verified calcGrowth + computeNominalDeviationGrowth + computePriceEffect imports remain used elsewhere.
- src/app/api/analysis/route.ts — Edit 1: import line trimmed from `generateNarrative, buildRecommendations` to `buildRecommendations`. Edit 2: removed `topAnomaliesForNarrative: Array<{...}> = []` declaration (was lines 356-359). Edit 3: removed `if (topAnomaliesForNarrative.length < 5 && top.severity !== 'NORMAL') { topAnomaliesForNarrative.push({...}) }` block (was lines 405-415). Edit 4: removed `narrativeInput` construction + `narrativePromise = generateNarrative(narrativeInput);` (was lines 628-644). Edit 5: removed `const { narrative, source: narrativeSource } = await narrativePromise;` (was line 760). Edit 6: removed `narrative, narrativeSource,` from response data object (was lines 786-787). KEPT: buildRecommendations + recommendations in response (rule-based). KEPT: breakdownEnriched (still used by deviationBreakdown field).
- src/components/dashboard/ExportDialog.tsx — removed 3 SECTIONS entries: `{ key: 'aiSummary', label: 'Ringkasan Eksekutif', ... }`, `{ key: 'aiInsight', label: 'Observasi & Pola Deviasi', ... }`, `{ key: 'narrative', label: '15. Narasi Analisis', ... }`. KEPT: all 16 other rule-based sections including recommendations (Section 16), historical (Section 19), dqIssues (Section 18).
- src/app/page.tsx — Edit 1: import `import { NarrativePanel, RecommendationPanel } from '@/components/dashboard/Narrative';` → `import { RecommendationPanel } from '@/components/dashboard/Narrative';`. Edit 2: removed `Brain,` from lucide-react icons import. Edit 3: removed "Narasi AI" badge block (was lines 253-257): `{analysis.data?.narrativeSource === 'llm' && (<Badge ...><Brain .../> Narasi AI</Badge>)}`. Edit 4: changed Utama tab section comment `{/* Section: Narrative + Recommendation */}` → `{/* Section: Recommendation */}` and removed `<NarrativePanel data={analysis.data} />` line. Edit 5: same change in Insight tab — removed `<NarrativePanel data={analysis.data} />`. KEPT: both `<RecommendationPanel data={analysis.data} />` usages (rule-based). KEPT: ExtraCharts imports + all visual sections (3 donuts, ranking visual, outlet radar, cumulative deviation, area contribution bars) — verified unrelated to AI removal.
- src/components/dashboard/Narrative.tsx — REWROTE entire file (83→47 lines). Deleted: NarrativePanel component. Removed now-unused imports: ScrollArea, FileText, Sparkles, ReactMarkdown (all only used by NarrativePanel). KEPT: RecommendationPanel component (rule-based). KEPT: Card, CardContent, CardHeader, CardTitle, Badge, CheckCircle2, AlertCircle imports (still used by RecommendationPanel).
- src/hooks/useAnalysis.ts — removed `narrative: string;` and `narrativeSource: string;` fields from AnalysisData interface (was lines 108-109). All other fields preserved.
- Verified no dangling references: `grep -r "NarrativePanel|generateNarrative|generateAIExecutiveSummary|generateAIPatternInsight|NarrativeInput|buildFallbackNarrative|buildStructuredSummary|narrativeInput|aiSummary|aiInsight|narrativeSource"` across /src returned 0 matches. Remaining `narrative` references in codebase are all unrelated rule-based fields (rules.yaml `narrative_template`, evaluator.ts `narrativeTemplate`/`top.narrative`, inventory.ts `narrative: string` types for rule evaluation results) — correctly preserved.
- Restored unrelated files damaged by sandbox reset during this task: src/components/dashboard/ExtraCharts.tsx (was deleted, restored via `git checkout HEAD --` since page.tsx imports 9 chart components from it), src/components/dashboard/AnalysisCards.tsx (had broken imports missing `GitBranch` and `Cell` — restored to HEAD).
- Final verification: `bun run lint` → 0 errors; `npx tsc --noEmit --skipLibCheck` → 0 errors. Both checks pass cleanly.

Stage Summary:
- src/engine/narrative/narrative.ts — 438 lines → 74 lines (364 lines removed: 12 AI functions/constants/interfaces/helpers deleted; buildRecommendations preserved)
- src/app/api/export-report/route.ts — 982 lines → ~920 lines (~62 lines removed: 1 import trimmed, 1 narrativeInput+promises block, 1 await+destructure, 4 data fields, 3 render blocks, 1 unused renderMarkdownLine helper, 1 topAnomaliesForNarrative declaration+push)
- src/app/api/analysis/route.ts — 826 lines → ~780 lines (~46 lines removed: 1 import trimmed, 1 topAnomaliesForNarrative declaration+push, 1 narrativeInput+narrativePromise block, 1 await, 2 response fields)
- src/components/dashboard/ExportDialog.tsx — 3 SECTIONS array entries removed (aiSummary, aiInsight, narrative)
- src/app/page.tsx — 5 edits: 1 import trimmed, 1 icon removed, 1 badge block removed (5 lines), 2 NarrativePanel JSX usages removed + 1 comment updated
- src/components/dashboard/Narrative.tsx — 83 lines → 47 lines (36 lines removed: NarrativePanel component + 4 now-unused imports)
- src/hooks/useAnalysis.ts — 2 fields removed from AnalysisData interface (narrative, narrativeSource)
- Total: ~515 lines removed across 7 files. lint + tsc both pass with 0 errors. KEPT: buildRecommendations, RecommendationPanel, Section 16 Rekomendasi, Section 19 Historical Anomaly, queryItemConsistency (all rule-based or user-explicitly-preserved).

---
Task ID: REMOVE-CHARTS
Agent: Charts Remover
Task: Remove ExtraCharts + RestoBahanMatrix

Work Log:
- Read worklog.md (last ~80 lines) for context — prior FIX-DEEP-1 and AUDIT-GROWTH-SIGNED entries confirmed resto-bahan-matrix API route existed at src/app/api/resto-bahan-matrix/route.ts and was referenced by RestoAnalysis.tsx + 6 query-invalidation sites.
- Read all 8 target files BEFORE editing: ExtraCharts.tsx (854 lines, 9 exports), page.tsx, RestoAnalysis.tsx (842 lines), QuickSettings.tsx, SettingsDialog.tsx, FilterBar.tsx, month-resolver.ts, useAnalysis.ts (confirmed no RestoBahanMatrix types in useAnalysis).
- Deleted src/components/dashboard/ExtraCharts.tsx (entire file, 854 lines, 9 chart components: HealthDistributionDonut, DeviationCategoryDonut, AreaContributionBar, TopItemsHorizontalBar, VarianceDivergingBar, OutletRadarChart, DirectionDistributionPie, CumulativeDeviationArea, AreaLossSalesComparison).
- Deleted src/app/api/resto-bahan-matrix/route.ts + route directory (303 lines).
- Edited src/app/page.tsx: removed ExtraCharts import block (lines 11-21), removed TrendingUp + PieChart as PieChartIcon from lucide-react import (both became unused after section removals), removed 9 ExtraCharts JSX usages across Dashboard/Insight/Area/Cost tabs — removed entire "Distribusi Visual" section (3 donuts), "Cumulative Deviation" sections (Dashboard + Cost tabs), "Ranking Visual" section, "Outlet Radar" sections (Dashboard + Area tabs), "Kontribusi Area" section, Insight tab chart sections + standalone charts, Cost tab TopItemsHorizontalBar+AreaContributionBar section.
- Edited src/components/dashboard/RestoAnalysis.tsx: deleted RestoBahanMatrix function (lines 673-824, ~152 lines incl. comment header), removed `<RestoBahanMatrix />` JSX usage (lines 272-275), removed Grid3x3 + ChevronRight from lucide-react import (Grid3x3 was only used in RestoBahanMatrix; ChevronRight was pre-existing unused). KEPT selectedItem state — still used by MenuAnalysis (line 269), ranking table rows (line 314), and ItemDetailModal (lines 349-355). KEPT RestoAnalysis tab itself + all other sub-components (MenuAnalysis, ItemDetailModal, SummaryCard, Row, rankings table, outlet profile cards).
- Edited src/components/dashboard/QuickSettings.tsx: removed `queryClient.invalidateQueries({ queryKey: ['resto-bahan-matrix'], refetchType: 'active' })` (line ~131, inside saveMutation onSuccess).
- Edited src/components/filters/SettingsDialog.tsx: removed 2 `resto-bahan-matrix` invalidations (saveMutation onSuccess line ~139 + resetMutation onSuccess line ~174).
- Edited src/components/filters/FilterBar.tsx: removed 2 `resto-bahan-matrix` invalidations (handleIngest line ~129 + handleDriveImport line ~192).
- Edited src/lib/month-resolver.ts: updated comment header to remove "resto-bahan-matrix" from the list of routes using the shared resolver (the route no longer exists).
- Encountered transient state during `git stash` operations (used to verify pre-existing tsc errors) — auto-formatter temporarily removed GitBranch import from AnalysisCards.tsx; resolved by `git checkout` restoring the file to HEAD. Re-applied page.tsx + ExtraCharts.tsx deletion after stash pop restored working tree.
- Ran `bun run lint` → exit 0, 0 errors.
- Ran `npx tsc --noEmit --skipLibCheck` → exit 0, 0 errors.
- Verified with grep: 0 references to ExtraCharts/HealthDistributionDonut/OutletRadarChart/etc. in page.tsx; 0 references to RestoBahanMatrix/Grid3x3 in RestoAnalysis.tsx; 0 `resto-bahan-matrix` invalidations in QuickSettings/SettingsDialog/FilterBar.

Stage Summary:
- **DELETED** src/components/dashboard/ExtraCharts.tsx — 854 lines, 9 chart components removed entirely.
- **DELETED** src/app/api/resto-bahan-matrix/route.ts (+ route directory) — 303 lines removed.
- src/app/page.tsx — 117 lines changed (net ~107 removed): ExtraCharts import block (10 lines) + TrendingUp/PieChartIcon imports + 9 ExtraCharts JSX sections/standalone usages across 4 tabs.
- src/components/dashboard/RestoAnalysis.tsx — 160 lines changed (net ~158 removed): RestoBahanMatrix function (~152 lines) + JSX usage (4 lines) + Grid3x3/ChevronRight imports. RestoAnalysis tab + MenuAnalysis + ItemDetailModal + rankings table + outlet profile all preserved.
- src/components/dashboard/QuickSettings.tsx — 1 line removed (resto-bahan-matrix invalidation in saveMutation).
- src/components/filters/SettingsDialog.tsx — 2 lines removed (resto-bahan-matrix invalidations in saveMutation + resetMutation).
- src/components/filters/FilterBar.tsx — 2 lines removed (resto-bahan-matrix invalidations in handleIngest + handleDriveImport).
- src/lib/month-resolver.ts — 2 lines (1 removed, 1 added): comment updated to drop resto-bahan-matrix from resolver-route list.
- Total: ~1,431 lines removed (1,157 from deleted files + ~274 from modified files).
- Note: 3 other dialog files (PicManagementDialog.tsx:202, DataManagementDialog.tsx:195, FileUploadDialog.tsx:475) still contain orphaned `queryClient.invalidateQueries({ queryKey: ['resto-bahan-matrix'] })` calls — left untouched per task instruction "only touch files listed above". These are dead no-op calls (query key no longer exists) but cause no compile/lint errors.
- `bun run lint` → 0 errors. `npx tsc --noEmit --skipLibCheck` → 0 errors.

---
Task ID: REMOVE-PARETO-PRICE
Agent: Pareto + Price Effect Remover
Task: Remove Pareto section + Price Effect decomposition

Work Log:
- Read worklog.md (last 50 lines) for context — confirmed prior REMOVE-AI task had trimmed narrative/AI imports in analysis + export-report routes; verified those changes are intact and unrelated to current task.
- Read all target files: src/lib/queries/items.ts (455 lines, queryPareto at 299-393), src/lib/metrics/growth.ts (192 lines, PriceEffectResult at 142-155 + computePriceEffect at 167-191), src/app/api/analysis/route.ts (826 lines), src/app/api/export-report/route.ts (982 lines), src/components/dashboard/ExportDialog.tsx, src/hooks/useAnalysis.ts, src/components/dashboard/AnalysisCards.tsx, src/components/dashboard/CostAccounting.tsx, src/components/dashboard/InsightsPanel.tsx, src/components/dashboard/Charts.tsx, src/types/inventory.ts.
- Verified calcAvgPrice is still used by src/engine/analysis/ruleService.ts (per-record rule context priceGrowth) — KEPT calcAvgPrice, only removed computePriceEffect + PriceEffectResult.
- Verified nominalDeviasiGrowthMagnitude is still used by growthMetrics.nominalDeviasiGrowth (KEEP) — only removed its usage in computePriceEffect call.
- Verified GrowthMetrics interface in src/types/inventory.ts is used by DashboardData (defined but DashboardData itself is currently unused) — still removed priceGrowth field for type cleanliness.
- src/lib/queries/items.ts — deleted queryPareto function (95 lines) + updated header comment to drop "Pareto" mention.
- src/lib/metrics/growth.ts — deleted PriceEffectResult interface (14 lines) + computePriceEffect function (25 lines) + their explanatory comment block (16 lines).
- src/lib/metrics/index.ts — removed `computePriceEffect` and `type PriceEffectResult` from growth barrel export; updated comment from "price effect decomposition" to "signed vs abs growth".
- src/lib/queries.ts — updated comment to remove "pareto" from items.ts description.
- src/app/api/analysis/route.ts — Edit 1: import `calcGrowth, computePriceEffect, computeNominalDeviationGrowth` → `calcGrowth, computeNominalDeviationGrowth`. Edit 2: removed `queryPareto,` from queries import. Edit 3: removed `paretoSql,` from Promise.all destructuring + `queryPareto(week!, month!, filterOpts, 50),` call (Group 2 SQL queries). Edit 4: removed `currAvgPrice` / `prevQtyDev` / `prevAvgPrice` computation block (lines 504-514) — only used by priceEffectResult. KEPT `nominalDeviasiGrowthMagnitude` (used by growthMetrics.nominalDeviasiGrowth). Edit 5: removed `priceEffectResult = computePriceEffect(...)` call + Phase 4 comment. Edit 6: removed `priceGrowth`, `volumeEffect`, `priceEffect`, `operationalEffect` from growthMetrics object. Edit 7: removed `paretoItems` + `pareto` object construction (lines 682-697). Edit 8: removed `pareto,` from response data. Edit 9: updated top-of-file comment from "Trend, exec summary, top items/outlets, pareto, etc." → "...etc.".
- src/app/api/export-report/route.ts — Edit 1: import `calcGrowth, computePriceEffect, computeNominalDeviationGrowth` → `calcGrowth, computeNominalDeviationGrowth`. Edit 2: removed `queryPareto,` from queries import. Edit 3: removed `paretoSql,` from Promise.all destructuring + `queryPareto(week, month, filterOpts, 50),` call. Edit 4: removed `currAvgPrice` / `prevQtyDev` / `prevAvgPrice` computation block. KEPT `nominalDeviasiGrowthMagnitude`. Edit 5: removed `priceEffectResult = computePriceEffect(...)` call. Edit 6: removed `priceGrowth`, `volumeEffect`, `priceEffect`, `operationalEffect` from growthMetrics object. Edit 7: removed `paretoItems` + `pareto` object construction. Edit 8: removed `pareto,` from data object. Edit 9: Section 3 "ANALISIS PERUBAHAN (GROWTH)" — replaced paragraph description from "Perubahan antar periode. Pengaruh Volume = ... Pengaruh Harga = ... Pengaruh Operasional = ..." to "Perubahan antar periode untuk metrik kunci (Sales, BOM, QTY Deviasi, Nominal Deviasi)." Edit 10: Section 3 growth table — removed 4 rows: 'Price Growth', 'Volume Effect', 'Price Effect', 'Operational Effect'. KEPT 4 rows: Penjualan Growth, QTY BOM Growth, QTY Deviasi Growth, Nominal Deviasi Growth. Edit 11: removed Section 10 "ANALISIS PARETO (ABC)" render block entirely (`if (hasSection('pareto')) { ... }` + heading + paragraph + table).
- src/components/dashboard/ExportDialog.tsx — Edit 1: removed `{ key: 'pareto', label: '10. Pareto (ABC)', description: 'Class A count + top 20 items dengan cumulative %', default: true }` from SECTIONS array. Edit 2: updated growth section description from "8 growth metrics + multi-period comparison + price effect" to "4 growth metrics + multi-period comparison".
- src/hooks/useAnalysis.ts — Edit 1: removed `ParetoResult` interface (7 lines). Edit 2: removed `priceGrowth: number | null;` from growthComparison type. Edit 3: removed `pareto?: ParetoResult;` from AnalysisData interface.
- src/components/dashboard/AnalysisCards.tsx — Edit 1: removed `GitBranch` from lucide-react imports (only used by TrendDecompositionCard). Edit 2: removed `Cell` from recharts imports (only used by TrendDecompositionCard). Edit 3: deleted entire TrendDecompositionCard component (~118 lines, including 2.2 header comment, function definition, waterfall chart, 3-card breakdown).
- src/app/page.tsx — Edit 1: removed `TrendDecompositionCard` from AnalysisCards import. Edit 2: removed `ParetoAnalysis` from CostAccounting import. Edit 3: removed `GitBranch` from lucide-react imports. Edit 4: removed Trend Decomposition section (`<section>` + SectionHeader + `<TrendDecompositionCard data={analysis.data} />`). Edit 5: changed Cost Accounting grid from `lg:grid-cols-2` (with ParetoAnalysis) to `lg:grid-cols-1` (CostImpactDecomposition only) and removed `<ParetoAnalysis data={analysis.data} />` line.
- src/components/dashboard/CostAccounting.tsx — Edit 1: removed `TrendingDown` from lucide-react imports (only used by ParetoAnalysis). Edit 2: deleted `classBadge` helper function (7 lines, only used by ParetoAnalysis). Edit 3: deleted `classifyByCumPct` helper function (6 lines, only used by ParetoAnalysis). Edit 4: deleted entire ParetoAnalysis component (~91 lines, including 6.2 header comment, function definition, table, badges).
- src/components/dashboard/InsightsPanel.tsx — Edit 1: removed `Target` from lucide-react imports (only used by pareto insight). Edit 2: removed pareto insight block (`// ----- 4. Pareto -----` + 11 lines checking `data.pareto` + pushing 'Aturan Pareto Berlaku' insight). Edit 3: renumbered subsequent insight comments from 5-10 → 4-9 for sequential consistency.
- src/components/dashboard/Charts.tsx — removed `{ name: 'Price', growth: g.priceGrowth },` from chartData array in GrowthComparison component. Remaining 4 bars (Sales, BOM, QTY Deviasi, Nominal Deviasi) are unchanged; `.filter((d) => d.growth != null)` continues to skip null values.
- src/types/inventory.ts — removed `priceGrowth: number | null;` from GrowthMetrics interface (kept salesGrowth, bomGrowth, qtyDeviasiGrowth, nominalDeviasiGrowth, deviationToSalesRatio, deviationToBomRatio).
- Verified zero dangling references: `grep -rn "queryPareto|computePriceEffect|PriceEffectResult|TrendDecompositionCard|ParetoAnalysis|ParetoResult|paretoSql|priceEffectResult|g\.priceGrowth|g\.volumeEffect|g\.operationalEffect|data\.pareto|pareto\.classACount|[Pp]areto" src/` returned 0 matches.
- Verified KEPT items intact: growthMetrics object (salesGrowth, bomGrowth, qtyDeviasiGrowth, nominalDeviasiGrowth, deviationToSalesRatio, deviationToBomRatio, multiPeriodComparison), Section 3 Growth (4 rows preserved), calcGrowth, calcGrowthAbs, computeNominalDeviationGrowth, computeGrowthResult, calcAvgPrice (still used by ruleService.ts), safeRatio, Historical Anomaly section, Google Drive Import.
- Final verification: `bun run lint` → 0 errors (LINT_EXIT=0); `npx tsc --noEmit --skipLibCheck` → 0 errors (TSC_EXIT=0). Both checks pass cleanly.

Stage Summary:
- src/lib/queries/items.ts — 455 lines → 360 lines (95 lines removed: queryPareto function + header comment word "Pareto")
- src/lib/metrics/growth.ts — 192 lines → 124 lines (68 lines removed: PriceEffectResult interface + computePriceEffect function + multiplicative decomposition comment block)
- src/lib/metrics/index.ts — 72 lines → 70 lines (2 lines removed: computePriceEffect + PriceEffectResult exports; 1 comment word changed)
- src/lib/queries.ts — 1 comment word changed (pareto → removed from items.ts description)
- src/app/api/analysis/route.ts — 826 lines → 760 lines (~66 lines removed: queryPareto import + call + destructuring + pareto object construction + pareto response field + computePriceEffect import + currAvgPrice/prevQtyDev/prevAvgPrice block + priceEffectResult call + 4 growthMetrics fields (priceGrowth, volumeEffect, priceEffect, operationalEffect) + 2 comments)
- src/app/api/export-report/route.ts — 982 lines → ~895 lines (~87 lines removed: same as analysis route + Section 3 4-row removal + Section 3 description simplification + Section 10 entire pareto render block)
- src/components/dashboard/ExportDialog.tsx — 1 SECTIONS array entry removed (pareto checkbox); 1 description text updated (growth section)
- src/hooks/useAnalysis.ts — ParetoResult interface (7 lines) + pareto field + priceGrowth field removed
- src/components/dashboard/AnalysisCards.tsx — 391 lines → 271 lines (120 lines removed: TrendDecompositionCard component + GitBranch + Cell imports)
- src/app/page.tsx — TrendDecompositionCard + ParetoAnalysis imports + Trend Decomposition section + ParetoAnalysis JSX usage removed; GitBranch removed from imports; Cost Accounting grid changed from 2-col to 1-col
- src/components/dashboard/CostAccounting.tsx — 501 lines → 391 lines (110 lines removed: ParetoAnalysis component + classBadge + classifyByCumPct helpers + TrendingDown import)
- src/components/dashboard/InsightsPanel.tsx — pareto insight block (~11 lines) + Target import removed; 5 insight comments renumbered
