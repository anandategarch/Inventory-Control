...
- `GET /api/peer-comparison/top-items?outletCode=&month=&week=&mode=&topN=&limit=&kelompok=` — **PEERTOP**: top item tiap resto setara (satu definisi “top item” = agregat per (outlet,item) `SUM(absNominalDeviasi)`, RANK via ROW_NUMBER) + union lintas peer (peerTopCount, rata-rata ABSOLUTE, blind-spot target null) + perPeer per outlet (expand row Peer Table). **PEERTOP-R1 (user feedback)**: union row kini membawa `qtyDeviasi` NILAI ASLI (signed, minus = kekurangan → merah), `itemRank`/`itemOutletCount` = “Ranking Resto di antara Resto yang Selevel per Item” (RANK() PARTITION BY itemId, urut SUM(absNominal) DESC), `peerAvgAbsQty` (rata-rata ABSOLUTE berbasis |kuantiti deviasi| — menggantikan |nominal|), kolom Arah dihapus di kedua surface (UI + PDF). FE: card “Top Items Across Peers” (kolom Top di = NAMA resto setara) + expand row Peer Table. PDF: section 8.3 (8.4 per-outlet DIHAPUS atas permintaan user). **PEERTOP-R2 (user feedback)**: union row kini membawa `topDiNames` = TOP-3 NAMA resto urut SUM(|nominal deviasi|) DESC untuk item itu (kandidat = peer top-N carriers + baris target sendiri any-rank — nama target muncul tepat saat masuk top 3; menggantikan daftar semua carrier); SEMUA header yang memakai istilah generik "target" diganti NAMA outlet itu sendiri ("Rangking KWGGAL" / "Nominal KWGGAL" / "QTY Deviasi (KWGGAL)", 8.2 juga, FE card + items-table "Target" column ikut) — user: "Ganti istilah target jadi nama resto target itu sendiri ... pakai nama kwggal aja daripada Resto"; judul 8.3 = "Item di Resto lain (yang setara penjualan <nama>) jika dilihat dari TOP Item nya". Di-cache 5 mnt + in-flight dedup, cache key +sv:2 (shape berubah); `limit` HARUS sama dengan route utama agar band identik.
...
- `RestoRecommendationCard` — Restaurant recommendation card (**H-13**: signal `zScoreAbnormalCount` di-rename `highDevBomCount` di seluruh rantai (SQL q-outlet-agg → API /api/recommendations signals → badge UI sekarang "Deviasi >50% BOM", bukan klaim z-score — hitungan fixed-threshold |Dev/BOM|>0.50; `zScoreWarningCount` dead dihapus; cache key q-outlet-agg + recommendations diberi marker `v:2` supaya row shape lama tak pernah disajikan). **H-11 #4b**: fetch pindah ke hook bersama `src/hooks/useRecommendations.ts` — SATU query shape + limit (5) untuk kartu ini DAN Resto tab; RestoTab PEEK cache top-5 via `useSharedRecommendationForOutlet` → outlet fokus yang sudah ada di top-5 dashboard = 0 fetch ekstra; scope global outlet → dedup 1 request + 1 server cache row, sebelumnya limit 5 vs 1 = double compute)
